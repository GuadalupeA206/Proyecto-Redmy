...
def partida_bots():
    r = 0
    print("Iniciando una partida entre bots...")
    ...
    while True:
        ...
        if len(jugadasLegales) > 0:
            mejor_jugada = max(jugadasLegales, key=len)
            for ficha in mejor_jugada[:-1]:
                for f in lista_de_fichasBot[turno_actual - 1]:
                    if f.numero1 == ficha and f.cadena == mejor_jugada[-1]:
                        tablero.append(f)
                        lista_de_fichasBot[turno_actual - 1].remove(f)
        ...
        if len(lista_de_fichasBot[turno_actual - 1]) == 0:
            print(f"El Bot {turno_actual} ha ganado!")
            break
