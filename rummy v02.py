            ...
            if (has_numeric_value(JugadasBot)==True and len(JugadasBot)>0):
               ...
               # si esta vacia, siempre entrara la ficha
                ...
                if (len (jugadasc[JugadasBotName]) >0 and (jugadasc[JugadasBotName][-1]==JugadasBot[i]-1 or 
                    (jugadasc[JugadasBotName][-1]=="COM" and jugadasc[JugadasBotName][-2]==JugadasBot[i]-2))):
                                                           
                                                            
                   
                    jugadasc[JugadasBotName].append(JugadasBot[i])
                  # si no lo es ni es igual, se vacia la lista si es que no cumple con el tamaño minimo
                if (((jugadasc[JugadasBotName][-1]!=JugadasBot[i]-1 and 
                    jugadasc[JugadasBotName][-1]!=JugadasBot[i]) or (jugadasc[JugadasBotName][-1]=="COM"
           ...
           # Añadir fichas a la lista temporal
                    for element4 in lista_de_fichasBot[turno_actual - 1]:
                        for q in range(len(jugadasLegales[p])):
                            if (q < len(jugadasLegales[p]) and (element4.cadena == shrC or element4.cadena=="COM") and
                            element4.numero1 == jugadasLegales[p][q]):
                    ...
                    if (  len(jugadasLegales)>p and len(jugadasLegales[p])>3 ):
                        ...
                        for q in range(len(jugadasLegales[p])-2):
                            ...
                            for element5 in (lista_de_fichasBot[turno_actual-1]):
                                ...
                                if (f"{element5.numero1}"==shrC and len(jugadasLegales[p])>1 and
                                        ...
                                        for lista in listas:
                                            ...
                                            if (condicion3!=0):
                                                for element2 in (lista_de_fichasBot[turno_actual-1]):
                                                    ...
                                                    bot_index = turno_actual - 1  # Los índices de la lista comienzan desde 0
                                                    """
...
def partida_bots():
    ...
    # Añadir fichas a la lista temporal
                for element4 in lista_de_fichasBot[turno_actual - 1]:
                    for q in range(len(jugadasLegales[p])):
                        if q < len(jugadasLegales[p]) and element4.cadena == shrC and element4.numero1 == jugadasLegales[p][q]:
                            fichas_temporales.append(element4)
                            jugadasLegales[p].remove(jugadasLegales[p][q])
                            lista_de_fichasBot[turno_actual - 1].remove(element4)
                            break  # Salir del bucle interno para evitar errores de índice
