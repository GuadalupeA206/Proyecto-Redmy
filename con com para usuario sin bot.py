            ...
            if (has_numeric_value(JugadasBot)==True and len(JugadasBot)>0):
               ...
               # si esta vacia, siempre entrara la ficha
                ...
                if (len (jugadasc[JugadasBotName]) >0 and (jugadasc[JugadasBotName][-1]==JugadasBot[i]-1 or 
                    (jugadasc[JugadasBotName][-1]=="COM" and jugadasc[JugadasBotName][-1]==JugadasBot[i]-2))):
...
def Generar_pozo_de_fichas():
    ...
    for ficha in fichas_combinadas_ordenadas:
        ...
        if contcom in (createcom1 , createcom2):
             ...
             pila_de_fichas.extend([Ficha("COM","","")])
        contcom+=1
...
while True:
    ...
    if turno_actual == turno_jugador:
        ...
        turnos.enqueue(turno_actual)
        for sCom in lista_de_fichasBot[turno_actual-1]:
            if sCom=="COM":
                  cCom=1
            else:
                 cCom=0
        ...
        for i, ficha in enumerate(lista_de_fichasBot[turno_actual-1]):
            print(f"{i+1}. {ficha.cadena}{ficha.numero1}")
    ...
    jugadasLegales=determinarJugadas(lista_de_fichasBot[turno_actual-1],cCom)
    ...
    if (len(jugadasLegales)>0):
        ...
        if (shrC=='N' or shrC=='A' or shrC=='R' or shrC=='V'):
            for element4 in (lista_de_fichasBot[turno_actual-1]):
                ...
                if (shrC=='N' or shrC=='A' or shrC=='R' or shrC=='V'):
                    for q in range (0, len(jugadasLegales[p])-1):
                    
                        # la primera condicion es para evitar un error de out of bounds, despues se verifica si la 
                        # informacion de la ficha de la lista de fichas del bot cuadra con la que se esta buscando
                        # para la jugada, y si la hace la anexa al tablero y la borra del mazo del bot
                        # y de las fichas que se estan buscando
            ...
            # cuando lo unico que quede es el identificador, significa que todas las fichas de las jugadas fueron 
            # recuperadas y se puede anexar la jugada
                ...
                for element5 in (lista_de_fichasBot[turno_actual-1]):
                    ...
                    for q in range(len(jugadasLegales[p])-2):
                        ...
                        if (element5.numero1==shrC and element5.cadena == jugadasLegales[p][q]):
                                ...
                                lista_de_fichasBot[turno_actual-1].remove(element5)
                                print("1")
    ...
    if (checkDeckLen==len(lista_de_fichasBot[turno_actual-1]) and len(mi_pozo_de_fichas)>0):
        FichasJ=sacar_ficha_de_pila(mi_pozo_de_fichas) 
